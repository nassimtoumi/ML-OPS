...
def download_json_from_minio(minio_client, minio_bucket, json_path, local_path):
    """Download JSON file from MinIO to local path"""
    minio_client.fget_object(minio_bucket, json_path, local_path)
...
class Data(torch.utils.data.Dataset):
    ...
    def __init__(self, json_path, sample_rate, n_feats, specaug_rate, specaug_policy,
                time_mask, freq_mask, minio_client, minio_bucket, valid=False, shuffle=True, text_to_int=True, log_ex=True):
        self.log_ex = log_ex
        self.text_process = TextProcess()
        
        # Download the JSON file from MinIO
        local_json_path = "/tmp/" + os.path.basename(json_path)
        download_json_from_minio(minio_client, minio_bucket, json_path, local_json_path)
        
        print("Loading data json file from", local_json_path)
        self.data = pd.read_json(local_json_path, lines=True)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            local_file_path = "/tmp/clips/" + os.path.basename(file_path)

            # Télécharger le fichier depuis MinIO s'il n'est pas présent localement
            if not os.path.isfile(local_file_path):
                os.makedirs(os.path.dirname(local_file_path), exist_ok=True)
                self.minio_client.fget_object("waves", file_path, local_file_path)
                print(f"Téléchargé depuis MinIO : {file_path} vers {local_file_path}")

            # Charger le fichier audio depuis le chemin local
            waveform, _ = torchaudio.load(local_file_path)
            label = self.text_process.text_to_int_sequence(self.data['text'].iloc[idx])
            spectrogram = self.audio_transforms(waveform)  # (channel, feature, time)
            spec_len = spectrogram.shape[-1] // 2
            label_len = len(label)

            # Vérifier les conditions et les exceptions
            ...
            # Passer à l'index précédent ou suivant en cas d'erreur
